class UsersController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      ...
      if @event
        ...
        @event_user = EventUser.create(user_id: @user.id, event_id: @event.id,
                                       attended: true
        )

        if @user.save && @event.save && @event_user.save
          format.html do
            redirect_to(user_path(current_user.id), notice: 'Attendance was successfully updated.')
          end
  ...
  def payment
    ...
    # p @semester
    redirect_to(thank_you_user_path(current_user.id)) if @user[:has_paid_dues] == true
  ...
  def thank_you
    @user = User.find(params[:id])
    redirect_to(payment_user_path(current_user.id)) if @user[:has_paid_dues] == false
